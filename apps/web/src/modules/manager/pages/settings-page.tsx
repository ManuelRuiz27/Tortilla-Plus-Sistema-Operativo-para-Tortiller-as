...
export function SettingsPage() {
  ...
  if (isLoading) return <LoadingState message="Cargando configuracion..." />;
  if (isError || !data) return <p className="rounded-md border border-tp-border bg-white p-5 text-sm text-tp-danger">No se pudo cargar configuracion.</p>;

  return (
    <section>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wide text-tp-primary">Configuracion</p>
          <h1 className="mt-3 text-2xl font-semibold">Sucursal, POS y plan</h1>
          <p className="mt-2 text-sm text-tp-muted">Estado operativo, permisos y features activos.</p>
        </div>
      ...
      <div className="mb-5 grid gap-4 lg:grid-cols-3">
        ...
        <article className="rounded-md border border-tp-border bg-white p-5">
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            <StatusBadge tone={subscriptionStatus === "active" ? "success" : "warning"}>{subscriptionStatus}</StatusBadge>
          </div>
        </article>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <article className="rounded-md border border-tp-border bg-white p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold">Dispositivos POS</h2>
          </div>
          <div className="space-y-3">
            {data.posDevices.map((device) => (
              <div className="flex items-center justify-between border-t border-tp-border pt-3 first:border-t-0 first:pt-0" key={device.id}>
                ...
                <StatusBadge tone={device.status === "active" ? "success" : "warning"}>{device.status}</StatusBadge>
              </div>
            ))}
          </div>
        </article>
        ...
        <article className="rounded-md border border-tp-border bg-white p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold">Configuracion de paquetes</h2>
          </div>
          <div className="space-y-3">
        ...
        <article className="rounded-md border border-tp-border bg-white p-5">
          <h2 className="mb-4 text-sm font-semibold">Features activas</h2>
          <div className="flex flex-wrap gap-2">
            {features.length === 0 ? <p className="text-sm text-tp-muted">Sin features cargadas.</p> : null}
            {features.map((feature) => (
              <StatusBadge key={feature} tone="info">{feature}</StatusBadge>
            ))}
          </div>
        </article>
        ...
        <article className="rounded-md border border-tp-border bg-white p-5 xl:col-span-2">
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            {user?.roles.map((role) => <StatusBadge key={role} tone="neutral">{role}</StatusBadge>)}
          </div>
          <div className="flex flex-wrap gap-2">
            {user?.permissions.length === 0 ? <p className="text-sm text-tp-muted">Sin permisos cargados.</p> : null}
            {user?.permissions.map((permission) => <StatusBadge key={permission} tone="info">{permission}</StatusBadge>)}
          </div>
        </article>
      </div>
    </section>
